...
import { LEGION_CONFIG, MANAGER_CONFIG } from '../config/legion-config.mjs';
import { SLIPPAGE_BUFFERS } from '../config/trading-config.mjs';
...
class LegionManager extends EventEmitter {
  ...
  reallocateLegion(portfolioSummary) {
    ...
    this.shadowLegion.forEach((shadow, index) => {
      ...
      if (shadow.killMe || idleHours > MANAGER_CONFIG.CULL_THRESHOLD_HOURS || ageHours > MANAGER_CONFIG.STALE_GENOME_HOURS) {
  ...
  deploySwarm(asset, count, portfolioSummary) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      shadow.priceHistoryBuffer = this.liveEngine.priceHistoryBuffer;
  ...
  async marchLegion(portfolioSummary) {
    ...
    await Promise.all(this.shadowLegion.map(async (shadow) => {
      ...
      if (result.killMe) shadow.killMe = true;
      if (result.anyTradesThisCycle) {
        shadow.lastTradeTime = Date.now();
        this.handleShadowVictory(shadow);
      }
    }));
  }

  handleShadowVictory(shadow) {
  ...
  requestOptimization(asset) {
    ...
    this.activeDreamJobs.add(asset);
