...
// Volatility is a state variable: a separate Kalman filter with volatility-scale priors
// (vol ∈ [0.01, 0.15] in percentage units) that survives across cycles and across
// engine reconstruction. The slip filter `_kalman` and the volatility filter `volatilityKalman`
// are deliberately NOT the same instance — the observation noise scales differ by ~10×.
const volatilityKalman = new MultiAssetKalman({ q: 1e-6, r: 1e-5, x0: 0.025, p0: 0.05 });
// Phase-transition detector: sits one layer above MultiAssetKalman.classifyRegime and
// fires exactly-once regime transitions on sustained variance shifts, gating volatility
// filter resets so a single noisy cycle never trips a "regime changed" reaction.
const regimeDetector2 = createRegimeDetector({
  confirmK: 4,        // need 4-of-8 confirmations in the rolling observation window
  windowN: 8,
  minResidencyMs: 30_000, // prior phase must have been resident >= 30s before flipping
});
...
/**
 * Advance volatility as a state variable. Pulls one Kalman step on the
 * dedicated volatility filter (separate from the slip filter), persists the
 * resulting smoothed estimate and regime classification onto the engine's
 * state maps. Returns the smoothed volatility as a fraction (e.g. 0.025 = 2.5%).
 *
 * @param {TradingEngine} engine - the engine instance (state lives on it)
 * @param {string} sym - bare ticker (e.g. 'BTC')
 * @param {number} rawVol - raw observed volatility, fraction (e.g. realized vol)
 * @returns {{x: number, p: number, regime: 'STABLE'|'EXPANDING'|'COMPRESSING'}}
 */
export function observeVolatility(engine, sym, rawVol) {
  if (!Number.isFinite(rawVol) || rawVol <= 0) return null;
  const estimate = volatilityKalman.observe(sym, rawVol);
  const state = volatilityKalman.estimate(sym);
  const regime = volatilityKalman.classifyRegime(sym);
  engine.filteredVolatility[sym] = estimate;
  // Canonical regime channel is `engine.regimeState[sym].phase` (hysteresis-gated).
  // The raw classifier output stays available via the return value's `regime` field
  // for callers that want the unresolved label, but is no longer mirrored on the engine.
  return { x: estimate, p: state.variance || state.stddev || 0, regime };
}

/**
 * Scale a trigger (or any base number) by the symbol's filtered volatility and the
 * genome's TRIGGER_VOL_SENSITIVITY coefficient. Single-knob, single-consumer pattern:
 * the only place filteredVolatility is read from a decision path's perspective.
 *
 * Math: out = base * (1 + filteredVol * TRIGGER_VOL_SENSITIVITY).
 *
 * @param {TradingEngine} engine
 * @param {string} sym - bare ticker
 * @param {number} base - the base trigger value
 * @returns {number} - scaled value. If filteredVol is unavailable (no observations yet),
 *                     returns base unchanged (no scaling on cold-start).
 */
export function volScaledTrigger(engine, sym, base) {
  if (!Number.isFinite(base)) return base;
  const fv = engine.filteredVolatility[sym];
  if (!Number.isFinite(fv)) return base;
  const sensitivity = engine.genome && Number.isFinite(engine.genome.TRIGGER_VOL_SENSITIVITY)
    ? engine.genome.TRIGGER_VOL_SENSITIVITY
    : 2.8;  // default matches constants.mjs
  return base * (1 + fv * sensitivity);
}
...
export class TradingEngine {
  constructor(genome, modeParam = 'sim', initialCapital = 0, initialHoldings = {}) {
    this.genome = { ...genome };
    // mode param is now a simple hint. The live engine is what reads the adapter;
    // SIM is the default for offline harnesses.
    ...
    this.executor = modeParam === 'live' || modeParam === 'LIVE' ? 'live' : 'sim';
    ...
    this.priceHistoryBuffer = []; // Global high-res history; reused by the defective-shadow termination signal

    // --- Volatility state ---
    // `filteredVolatility` is a state variable keyed by symbol. Populated by
    // _observeVolatility(sym, rawVol), read by harvest/rebalance/spawn paths.
    // Survival across process restart is via loadPersistedState / getStateSnapshot
    // (volatilityKalmanState is the serialized Kalman state for each symbol).
    this.filteredVolatility = {};

    // Phase-transition state: per-symbol { phase, prev, transitionedAt, transitionsCount }.
    // Single canonical regime channel: `engine.regimeState[sym].phase` is the
    // hysteresis-gated output of the phase-transition detector. STABLE / EXPANDING /
    // COMPRESSING. Downstream consumers (RiskPolicy, decision paths) read this field only.
    this.regimeState = {};
    // --- Risk invariants ---
    // RiskPolicy gates every state-mutating action (BUY/SELL/spawn) through named
    // invariants. Branches that breach a cap are short-circuited with a one-event-per-cycle
    // log; the engine never executes an invariant-violating trade.
    this.riskPolicy = new RiskPolicy(this.genome);
    this._riskBreachesThisCycle = [];
  ...
  loadPersistedState(data) {
    ...
    if (data.filteredVolatility) this.filteredVolatility = data.filteredVolatility;
    if (data.volatilityKalmanState) volatilityKalman.restore(data.volatilityKalmanState);
    if (data.regimeState) this.regimeState = data.regimeState;
    if (data.regimePhaseDetectorState) regimeDetector2.restore(data.regimePhaseDetectorState);
  }
  ...
  getStateSnapshot() {
    return {
      ...
      // Dump volatility Kalman state so volatility survives process restart.
      volatilityKalmanState: volatilityKalman.serialize(),
      filteredVolatility: this.filteredVolatility || {},
      // Phase-transition detector state: rolling buffers + per-symbol state map.
      regimePhaseDetectorState: regimeDetector2.serialize(),
  ...
  /**
   * Feed a raw classifier label (STABLE/EXPANDING/COMPRESSING) into the phase-transition
   * detector. Promotes to a confirmed regime only when at least 4 of the last 8
   * observations agree AND the prior phase has been resident >= minResidencyMs.
   * Updates `this.regimeState[sym]` exactly once per confirmed transition.
   *
   * Returns { phase, transitioned, prev, inTransitionSince }.
   */
  _detectPhaseTransition(sym, rawLabel, now = Date.now()) {
    const result = regimeDetector2.observe(sym, rawLabel, now);
    this.regimeState[sym] = regimeDetector2.stateFor(sym);
    if (result.transitioned) {
      this._resetVolatilityFilterIfNeeded(sym);
    }
    return result;
  }

  /**
   * Reset the volatility Kalman filter for `sym` to a slightly elevated starting
   * volatility. ONLY called when the phase-transition detector has confirmed a
   * regime change (see _detectPhaseTransition). A noisy single-cycle label cannot
   * trip this path.
   */
  _resetVolatilityFilterIfNeeded(sym) {
    if (!volatilityKalman) return;
    const snap = volatilityKalman.serialize();
    if (snap[sym]) {
      // Re-anchor the prior estimate: shift toward an elevated starting vol so the
      // filter responds to the new regime faster than it would from the trailing mean.
      const stash = snap[sym];
      volatilityKalman.restore({
        ...snap,
        [sym]: { ...stash, x: Math.max(stash.x, 0.04) * 1.05, P: Math.max(stash.P || 0.05, 0.05) },
      });
    }
  }
  ...
  async update(portfolioSummary, api, cashBalance, holdingDetails, simTime = null, priceMap = null) {
    ...
    // --- Active Defective-Shadow Termination (Legion signal) ---
    if (this.isDefective(portfolioSummary, this.priceHistoryBuffer)) {
      return {
    ...
    if (!portfolioHarvestExecutedThisCycle) {
      for (const row of portfolioSummary) {
        ...
        const baseHarvestCycles = getGenomicParam(effectiveGenome, 'HARVEST_CYCLE_THRESHOLD', sym);
        ...
        if (flaggedDuration > effectiveGenome.FORCED_HARVEST_TIMEOUT) { shouldHarvest = true; harvestType = "Forced"; }
        ...
        if (shouldHarvest) {
          ...
          const minSurplus = harvestType === "Forced" ? effectiveGenome.MIN_SURPLUS_FOR_FORCED_HARVEST : getGenomicParam(effectiveGenome, 'MIN_SURPLUS_FOR_HARVEST', sym);

          if (surplus >= minSurplus) {
            let harvestTakePct = getGenomicParam(effectiveGenome, 'HARVEST_TAKE_PERCENT', sym);
            if (harvestTakePct === undefined) harvestTakePct = 0.70; // Fallback
            harvestTakePct = harvestTakePct * (1 + ((modulated?.conviction ?? 0) * 0.15));
            harvestTakePct = Math.max(0.3, Math.min(1.0, harvestTakePct));
            ...
            const crashFundThreshold = effectiveGenome.CRASH_FUND_THRESHOLD_PERCENT ?? 0.10;
                  ...
                  if (sellResp?.id) {
                    ...
                    if (this.mode === 'LIVE') {
                      this.postMortemEvents.push({
                        ...
                        genomeSlice: { ...effectiveGenome } // Pass full effective genome for now, Dreamer filters
                      });
                    }

                    const targetAdjust = getGenomicParam(effectiveGenome, 'TARGET_ADJUST_PERCENT', sym);
      ...
      for (const row of portfolioSummary) {
        ...
        if (now - rSt.triggeredAt > effectiveGenome.FORCE_REBALANCE_TIMEOUT) {
          const shortfall = rSt.currentBaselineWhenTriggered - totalVal;
          let buyUSD = shortfall * effectiveGenome.FORCE_REBALANCE_SHORTFALL_PERCENT;
          if (buyUSD >= effectiveGenome.MIN_FORCED_REBALANCE_USD) {
            ...
            if (minQty && parseFloat(qty) < minQty) {
              ...
              if (this.cashBalance >= requiredMinUSD) {
                ...
                if (this.mode === 'LIVE') {
                  console.log(`⚠️ [API Safety Adjustment] Up-sized forced rebalance for ${sym} from calculated quantity ${roundQty(sym, shortfall * effectiveGenome.FORCE_REBALANCE_SHORTFALL_PERCENT / curP)} to minimum required ${minQty} ($${requiredMinUSD.toFixed(2)})`);
                }
              }
            }
      ...
      // Snowball spawn cost: flat $30 floor. RiskPolicy below gates whether the proposed
      // trade is allowed; it does not change the engine's bottom-line spawn cost.
      // The two are separated on purpose: spawnCost is the engine's policy, the named
      // risk invariants are an enforced upper bound.
      const spawnCost = SNOWBALL_CONFIG.MIN_SPAWN_COST_USD || 30;
      ...
      if (nextSym) {
        // Resolve spawn price before gating so the policy can use asset-pct cap.
        const buyP_pre = (priceMap && priceMap[nextSym] && priceMap[nextSym] > 0)
          ? priceMap[nextSym]
          : (spawnPriceMap && spawnPriceMap[nextSym] && spawnPriceMap[nextSym] > 0)
            ...
            : (portfolioSummary.find(r => r.Symbol === nextSym)?.Price)
              || null;
        // RiskPolicy.assertAction is the single gate. It enforces named invariants:
        // CASH_FLOOR / MAX_VOL_PCT / MAX_ASSET_PCT / CASH_INSUFFICIENT.
        const symbolRegime = (this.regimeState && this.regimeState[nextSym]
          && this.regimeState[nextSym].phase) || 'STABLE';
        const riskCheck = this.riskPolicy.assertAction({
          kind: 'SPAWN',
          sym: nextSym,
          usd: spawnCost,
          cashBalance: this.cashBalance,
          totalPortfolioValue: currentTotalPortfolioValue,
          regime: symbolRegime,
          currentPrice: buyP_pre || null,
        });
        if (!riskCheck.allowed) {
          this._riskBreachesThisCycle.push({
            sym: nextSym, kind: riskCheck.breach.kind,
            observed: riskCheck.breach.observed,
            cap: riskCheck.breach.cap,
          });
        } else if (this.cashBalance >= crashFundUSD + spawnCost) {
          ...
          const buyP = buyP_pre || ((await api?.getQuotes?.([nextSym]))?.[nextSym]);
